...
(select lo_extendedprice*lo_discount as product, lo_orderdate from lineorder_opt where lo_discount between 1 and 3 and lo_quantity < 24) as t1,
(select d_datekey from dwdate_opt2 where d_year=1997) as t2
where t1.lo_orderdate=t2.d_datekey;
-- end query1_opt

-- start query2_opt
select sum(lo_revenue), d_year, p_brand1 from lineorder_opt, dwdate_opt2, part_opt, supplier_opt
where lo_orderdate = d_datekey 
and lo_partkey = p_partkey 
and lo_suppkey = s_suppkey
and p_category = 'MFGR#12' 
...
-- start query3_opt
select c_city, s_city, d_year, sum(lo_revenue) as revenue from 
(select lo_custkey, lo_orderdate, lo_revenue, lo_suppkey from lineorder_opt) as t4,
(select d_datekey, d_year from dwdate_opt2 where d_yearmonth = 'Dec1997') as t3,
...
-- start drop_tables_opt
drop table part_opt;
drop table supplier_opt;
drop table customer_opt;
drop table dwdate_opt;
drop table lineorder_opt;
-- end drop_tables_opt
...
CREATE EXTERNAL TABLE part_opt (
    ...
    p_size INT, 
    p_container STRING)
partitioned by (p_partkey int)
STORED AS PARQUET;
INSERT into table part_opt partition (p_partkey) select p_name, p_mfgr, p_category, p_brand1, p_color, p_type, p_size, p_container, p_partkey from part;
...
CREATE EXTERNAL TABLE supplier_opt (
    ...
    s_phone STRING)
STORED AS PARQUET;
INSERT into table supplier_opt select * from supplier;
...
CREATE EXTERNAL TABLE customer_opt (
    ...
    c_mktsegment STRING)
STORED AS PARQUET;
INSERT into table customer_opt select * from customer;
...
STORED AS PARQUET;
INSERT into table dwdate_opt partition (d_year) select d_datekey, d_date, d_dayofweek, d_month, d_yearmonthnum, 
 d_yearmonth, d_daynuminweek, d_daynuminmonth, d_daynuminyear, d_monthnuminyear, d_weeknuminyear, d_sellingseason
 d_lastdayinweekfl, d_lastdayinmonthfl, d_holidayfl, d_weekdayfl, d_year from dwdate;
...
CREATE EXTERNAL TABLE lineorder_opt (
    ...
    lo_shipmode STRING)
STORED AS PARQUET;
INSERT into table lineorder_opt select * from lineorder;
